...
public class MyGraph implements Graph {
	...
	public MyGraph(Collection<Vertex> v, Collection<Edge> e) {
		checkNull(v);
		checkNull(e);
		graph = new TreeMap<Vertex, Set<Edge>>();
		for (Vertex vertex : v)
	...
	*/
	public HashSet<Edge> getMST() {
		...
		while (disjEdges.numSets() > 1) {
			Edge edge = pq.remove();
			mst.add(edge);
			disjEdges.union(find(vertexTable(edge.getSource())), 
					find(vertexTable(edge.getDestination())));
		}
   ...
   public Path shortestPath(Vertex a, Vertex b) {
		...
		for (Vertex v : graph.keySet()) {
			...
			v.setKnown(false); //doesn't exist, need to correct in Vertex.java, also unnecessary because they are innitialized to false
			pq.insert(v);
		}
		Vertex current = a;
		if(pq.contains(b)){
			while(pq.contains(b) && !pq.isEmpty()) {
				for(Edge e : graph.get(current)) { 
	            Vertex destination = e.getDestination();
					int totalCost = current.getCost() + e.getWeight()
					destination.setCost(totalCost);
					pq.decreaseKey(destination, totalCost);
				}
